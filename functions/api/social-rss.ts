import socialConfig, { type SocialConfigItem } from '../../src/data/social';
...
export const onRequestGet: PagesFunction = async ({ request }) => {
  const slug = new URL(request.url).searchParams.get('slug')?.trim().toLowerCase() || 'substack';
  const config = socialConfig.find((entry: SocialConfigItem) => entry.slug === slug);
  ...
  try {
    ...
    return new Response(
      JSON.stringify({
        ok: true,
        items,
        message: `Showing ${items.length} latest item${items.length === 1 ? '' : 's'}.`
      }),
      {
        status: 200,
        headers: {
