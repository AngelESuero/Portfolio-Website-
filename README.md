...
### X feed auto-sync (`x-feed.json`)
`src/data/x-feed.json` is the canonical local source for homepage X items and `/social/x`.

It is auto-generated by `scripts/sync-x.mjs` and included in hourly sync via `npm run sync:data`.

#### Required GitHub secret (hourly workflow)
- Add repository secret:
  - `X_BEARER_TOKEN=<your X API bearer token>`
- Workflow:
  - `.github/workflows/sync-data.yml`
- Default handle:
  - `a_e_s_4` (set via `X_HANDLE`)

#### Local manual run
```bash
X_BEARER_TOKEN=... npm run sync:x
```

If `X_BEARER_TOKEN` is missing/invalid, or X API credits are depleted, `sync:x` now falls back to public embed syndication (`syndication.twitter.com`) before failing.

Optional overrides:
- `X_HANDLE` (default `a_e_s_4`)
- `X_MAX_ITEMS` (default `24`)

#### Failure behavior (last-known-good)
- If both X API and syndication fallback fail and existing `src/data/x-feed.json` is valid:
  - keep existing file unchanged
  - script exits successfully with warning
- If both X API and syndication fallback fail and no valid file exists:
  - write a minimal empty manifest
  - script exits with error

#### Cloudflare parity
Store the same secret in Cloudflare for parity/future runtime use:
- Secret name: `X_BEARER_TOKEN`

#### Important
`src/data/x-feed.json` is treated as auto-generated. Manual edits can be overwritten by sync.
